...
############## my stack class for path finding ##############################
class Stack:
...
# algorithm to find path to node
def Pathfind(nlist, dest, current_node, pathToDest, visited):

	current = current_node
	print("current node:" + str(current_node))
	visited.add(current)
	print("visited list: " + str(visited.peek()))
	unvisited = nlist[current] 
	pathToDest.add(current)
	print("current path to dest:" + str(pathToDest.peek()))
	count = 0

	#check if current node's neighbors match the dest node
	for crnt_node in unvisited:
		print("checking node:" + str(crnt_node))
		if int(crnt_node) == int(dest):
			print("Route to " + str(dest) + " Found via nodes: ")
			while pathToDest.length() > 0:
				print(str(pathToDest.remove()))
			return True
			# while True:
			# 	pop = pathToDest.remove()
			# 	if pop == False:
			# 		return False
			# 	else:
			# 		print(pop)
	...
	# if no nodes matched dest_nid then we can start searching the neighbors
	while count < 4:
		#let's call this function again but with the current nodes neighbors
		if unvisited[count] != 0:
			if int(unvisited[count]) != int(visited.peek()):
				return Pathfind(nlist, dest, unvisited[count], pathToDest, visited)
			count += 1

	# create a list of unvisited neighors
	print("define this function")
	# start with first neighbor 

	# return path unavailable if not possible
	return False # false means path is not available

# Function: sendto()
def send_tcp(dest_nid, message, neighbor_list, path, visited_list):

	# global variables
	global NID, hostname, tcp_port, neighbors
	global l1_hostname, l2_hostname, l3_hostname, l4_hostname
	global l1_tcp_port, l2_tcp_port, l3_tcp_port, l4_tcp_port
	global l1_NID, l2_NID, l3_NID, l4_NID
	...
	elif Pathfind(neighbor_list, dest_nid, NID, path, visited_list):
		# elif dest_nid == str(l4_NID): #if dest =! 0 then send
		HOST = l4_hostname
		PORT = l4_tcp_port
...
def send_udp(dest_nid, message):
	...
	elif dest_nid == str(l4_NID): #checking host table to see if 4 = 4
		HOST = l4_hostname
		PORT = l4_udp_port
	# if l4_NID == 0 then we should check neighboring nodes
...
def main(argv):
	...
	################# grabbing link table #################################
	nlist = node.Get_link_table()
	# code for debugging
	# print("node 2's neighbors: " + str(nlist[2]))
	# print("checking if the first neighbor is 1. if so it'll twerk")
	# if int(nlist[2][0]) == 1:
	# 	print("it twerked")
	...
	while(run):
		...
		# selection: send_tcp
			...
			send_tcp(dest_nid, message, nlist, pathToDest, visited_list)
			os.system("""bash -c 'read -s -n 1 -p "Press any key to continue..."'""")
